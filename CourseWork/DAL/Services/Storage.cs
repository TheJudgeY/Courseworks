...
namespace DAL.Services
{
    public class Storage<T> : IStorage<T> where T : BaseEntity
    {
        ...
        private async Task<int> GetMaxIdAsync()
        {
            ...
            {
                throw new InvalidOperationException($"Failed to get max Id. Exception: {ex.Message}");
            }
        }
        ...
        public async Task<Result<bool>> RemoveAsync(int id)
        {
            try
            {
                var items = await GetAllItemsAsync();
        ...
        public async Task<Result<bool>> UpdateAsync(int Id, T updatedObj)
        {
            try
            {
                var items = await GetAllItemsAsync();
